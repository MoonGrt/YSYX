...
extern char flash_payload_start[];
extern char flash_payload_end[];
...
int main(void) {
  uint32_t offset =
      (uint32_t)(uintptr_t)flash_payload_start - 0x30000000u;
  uint32_t size =
      (uint32_t)(uintptr_t)(flash_payload_end - flash_payload_start);
  volatile uint32_t *sram = (volatile uint32_t *)0x0f000400u;

  for (uint32_t i = 0; i < size; i += 4)
    sram[i / 4] = flash_read(offset + i);

  for (uint32_t i = 0; i < size; i += 4) {
    volatile uint32_t *xip =
        (volatile uint32_t *)(uintptr_t)(0x30000000u + offset + i);
    if (sram[i / 4] != *xip)
      __asm__ volatile(".word 0");
  }

  ((void (*)(void))(uintptr_t)sram)();
  ((void (*)(void))(uintptr_t)(0x30000000u + offset))();
  return 0;
}
