...
static void decode_operand(int *rd_, word_t *src1, word_t *src2, word_t *imm, int type) {
...
#define INSTPAT_MATCH(decode, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
  INSTPAT("0001110 ????? ????? ????? ????? ?????" , pcaddu12i, 1RI20 , R(rd) = decode.pc + imm);
  INSTPAT("0010100010 ???????????? ????? ?????"   , ld.w     , 2RI12 , R(rd) = Mr(src1 + imm, 4));
  INSTPAT("0010100110 ???????????? ????? ?????"   , st.w     , 2RI12 , Mw(src1 + imm, 4, R(rd)));

  INSTPAT("0000 0000 0010 10100 ????? ????? ?????", break    , N     , MEMUTRAP(decode.pc, R(4))); // R(4) is $a0
  INSTPAT("????????????????? ????? ????? ?????"   , inv      , N     , INV(decode.pc, decode.isa.inst));
  INSTPAT_END();
...
int isa_exec_once() {
  decode.isa.inst = inst_fetch(&decode.snpc, 4);
  return decode_exec();
}
