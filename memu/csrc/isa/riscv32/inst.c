...
#define INSTPAT_MATCH(decode, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 1100111", jalr  , I, decode.dnpc = (src1 + imm) & ~(uint32_t)0x1; R(rd) = decode.snpc;
  IFDEF(CONFIG_FTRACE, {
  ...
  INSTPAT("??????? ????? ????? ??? ????? 0010111", auipc , U, R(rd) = decode.pc + imm);
  ...
  INSTPAT("??????? ????? ????? ??? ????? 1101111", jal   , J, R(rd) = decode.pc + 4; decode.dnpc = decode.pc + imm;
  IFDEF(CONFIG_FTRACE, {
    if (rd == 1)  // x1: return address for jumps
      ftrace_call(decode.pc, decode.dnpc, false);
  }));

  INSTPAT("??????? ????? ????? 001 ????? 1100011", bne   , B, decode.dnpc = ((sword_t)src1) != ((sword_t)src2) ? decode.pc + imm : decode.dnpc);
  INSTPAT("??????? ????? ????? 000 ????? 1100011", beq   , B, decode.dnpc = ((sword_t)src1) == ((sword_t)src2) ? decode.pc + imm : decode.dnpc);
  INSTPAT("??????? ????? ????? 101 ????? 1100011", bge   , B, decode.dnpc = ((sword_t)src1) >= ((sword_t)src2) ? decode.pc + imm : decode.dnpc);
  INSTPAT("??????? ????? ????? 100 ????? 1100011", blt   , B, decode.dnpc = ((sword_t)src1) <  ((sword_t)src2) ? decode.pc + imm : decode.dnpc);
  INSTPAT("??????? ????? ????? 111 ????? 1100011", bgeu  , B, decode.dnpc = ((src1) >= (src2)) ? decode.pc + imm : decode.dnpc);
  INSTPAT("??????? ????? ????? 110 ????? 1100011", bltu  , B, decode.dnpc = src1 < src2 ? decode.pc + imm : decode.dnpc);

  INSTPAT("0000000 00001 00000 000 00000 1110011", ebreak, N, MEMUTRAP(decode.pc, R(10)));
  INSTPAT("0000000 00000 00000 000 00000 1110011", ecall , N, IFDEF(CONFIG_ETRACE, etrace(decode.pc, 11)); decode.dnpc = isa_raise_intr(decode.pc, 11));
  INSTPAT("0011000 00010 00000 000 00000 1110011", mret  , N, decode.dnpc = cpu.csr.mepc; cpu.csr.mstatus=0x80;);
