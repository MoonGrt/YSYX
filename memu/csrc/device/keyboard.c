...
#define MEMU_KEY_NAME(k) MEMU_KEY_ ## k,

enum {
  MEMU_KEY_NONE = 0,
  MAP(MEMU_KEYS, MEMU_KEY_NAME)
};

#define SDL_KEYMAP(k) keymap[SDL_SCANCODE_ ## k] = MEMU_KEY_ ## k;
static uint32_t keymap[256] = {};

static void init_keymap() {
  MAP(MEMU_KEYS, SDL_KEYMAP)
}
...
void send_key(uint8_t scancode, bool is_keydown) {
  if (nemu_state.state == MEMU_RUNNING && keymap[scancode] != MEMU_KEY_NONE) {
    uint32_t am_scancode = keymap[scancode] | (is_keydown ? KEYDOWN_MASK : 0);
    key_enqueue(am_scancode);
  }
}
#else // !CONFIG_TARGET_AM
#define MEMU_KEY_NONE 0
...
void init_i8042() {
  ...
  i8042_data_port_base[0] = MEMU_KEY_NONE;
