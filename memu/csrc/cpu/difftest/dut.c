...
// can not produce consistent behavior with MEMU
void difftest_skip_ref() {
  ...
  // already write some memory, and the incoming instruction in MEMU
...
// We should skip checking until MEMU's pc catches up with QEMU's pc.
// The semantic is
...
void init_difftest(char *ref_so_file, long img_size, int port) {
  if (ref_so_file == NULL) {
    Log("Differential testing: %s", ANSI_FMT("OFF", ANSI_FG_YELLOW));
    return;
  }
