// package soc.riscv.mini

// import chisel3._
// import chisel3.util._
// import soc.riscv.Instructions._
// import soc.riscv.Constants.MiniRV._
// import soc.util._
...
//     List(OP2.RS2, EX.ADD, GPR.EX, MEM.NONE), Array(
//       LW     -> List(OP2.IMI , EX.ADD , GPR.MEM , MEM.RW  ),  // x[rs1] + sext(immi)
//       LBU    -> List(OP2.IMI , EX.ADD , GPR.MEM , MEM.RBU ),  // x[rs1] + sext(immi)
//       SW     -> List(OP2.IMS , EX.ADD , GPR.NONE, MEM.WW  ),  // x[rs1] + sext(imms)
//       SB     -> List(OP2.IMS , EX.ADD , GPR.NONE, MEM.WB  ),  // x[rs1] + sext(imms)
//       ADD    -> List(OP2.RS2 , EX.ADD , GPR.EX  , MEM.NONE),  // x[rs1] + x[rs2]
//       ADDI   -> List(OP2.IMI , EX.ADD , GPR.EX  , MEM.NONE),  // x[rs1] + sext(immi)
//       JALR   -> List(OP2.IMI , EX.JAL , GPR.PC  , MEM.NONE),  // x[rd] <- PC+4 and (x[rs1]+sext(imm_i))&~1
//       LUI    -> List(OP2.IMU , EX.ADD , GPR.EX  , MEM.NONE),  // sext(immu[31:12] << 12)
//       EBREAK -> List(OP2.NONE, EX.NONE, GPR.NONE, MEM.NONE),
//     ),
//   )
...
//       (wbsel === GPR.PC ) -> (io.pc + 4.U),
//       (wbsel === GPR.EX ) -> io.exData,
//       (wbsel === GPR.MEM) -> memData,
//     ))
//   }
