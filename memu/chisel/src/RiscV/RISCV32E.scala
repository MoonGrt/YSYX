...
class Riscv32E_ID extends Module {
  ...
  val decoded = ListLookup(io.inst,
    List(OP1.RS1, OP2.RS2, EX.ADD, WB.EX, MEM_NONE, CSR1.NONE), Array(
      ...
      CSRRW  -> List(OP1.RS1 , OP2.NONE, EX.ADD , WB.CSR , MEM_NONE, CSR1.W   ), // CSRs[csr] <- x[rs1]
      CSRRWI -> List(OP1.IMZ , OP2.NONE, EX.ADD , WB.CSR , MEM_NONE, CSR1.W   ), // CSRs[csr] <- uext(imm_z)
      CSRRS  -> List(OP1.RS1 , OP2.NONE, EX.ADD , WB.CSR , MEM_NONE, CSR1.S   ), // CSRs[csr] <- CSRs[csr] | x[rs1]
      CSRRSI -> List(OP1.IMZ , OP2.NONE, EX.ADD , WB.CSR , MEM_NONE, CSR1.S   ), // CSRs[csr] <- CSRs[csr] | uext(imm_z)
      CSRRC  -> List(OP1.RS1 , OP2.NONE, EX.ADD , WB.CSR , MEM_NONE, CSR1.C   ), // CSRs[csr] <- CSRs[csr]&~x[rs1]
      CSRRCI -> List(OP1.IMZ , OP2.NONE, EX.ADD , WB.CSR , MEM_NONE, CSR1.C   ), // CSRs[csr] <- CSRs[csr]&~uext(imm_z)

      EBREAK -> List(OP1.NONE, OP2.NONE, EX.NONE, WB.NONE, MEM_NONE, CSR1.B   ),
      ECALL  -> List(OP1.NONE, OP2.CSR , EX.CSR , WB.NONE, MEM_NONE, CSR1.E   ),
      MRET   -> List(OP1.NONE, OP2.CSR , EX.CSR , WB.NONE, MEM_NONE, CSR1.MRET),
    ),
  )
  ...
  val brcsr = MuxCase(0.U(32.W), Seq(
    (csrsel === CSR1.E   ) -> CSR(3),  // mtvec
    (csrsel === CSR1.MRET) -> CSR(1),  // mepc
  ))
