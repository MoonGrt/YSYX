...
# compile and run x86-npc
  "ls /root/npc"
  "cp -r /root/npc $TMP_DIR"
  "export MEMU_HOME=$TMP_DIR/npc"
  "make -C $TMP_DIR/npc ISA=x86"
  "ls -lh /root/npc-prog"
  "file /root/npc-prog/amtest-x86-npc.elf"
  "$TMP_DIR/npc/build/x86-npc --batch --mainargs=h /root/npc-prog/amtest-x86-npc.bin"
  "file /root/npc-prog/microbench-x86-npc.elf"
  "$TMP_DIR/npc/build/x86-npc --batch --mainargs=test /root/npc-prog/microbench-x86-npc.bin"

# compile and run riscv64-npc
  "make -C $TMP_DIR/npc clean"
  "make -C $TMP_DIR/npc ISA=riscv64"
  "$TMP_DIR/npc/build/riscv64-npc --batch /root/npc-prog/linux-hello-riscv64-npc.bin"
)
...
* 当以可写方式启动镜像时, MEMU遇到错误或通过Ctrl+C直接退出MEMU时, 可能会损坏镜像的崩溃一致性, 此时可以通过fsck命令修复分区.
